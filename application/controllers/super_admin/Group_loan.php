        ...
        if ($this->form_validation->run()) {
            ...
            if ($roi_type == 1) {
                ...
                if ($tenure_type == 1 || $tenure_type == 2) {
                    ...
                    $lastdigit           = round($intrst_amt) % 10;
                    $final_intrst_amts   = round($intrst_amt) - $lastdigit;
                    $cala                = $lastdigit * $tenure;
                    $rest_intrst_amt     = $final_intrst_amts + $cala;
            ...
            $prin_amt = $amt / $tenure;
            $principal_amt = $amt;
            $emi_amt = round($intrst_amt) + round($prin_amt);
            $lastone    = round($emi_amt) % 10;  
            $final_emis = round($emi_amt) - $lastone;
            $cal = $lastone * $tenure;
            $rest_emi  = $final_emis + $cal;
            ...
            for ($i = 1; $i <= $tenure; $i++) {
                ...
                if ($i > 1) {
                    ...
                    if ($tenure_type == 1) {
                        ...
                        $Payment_start_date = date("Y-m-d", strtotime($Payment_start_date . $bi . 'day'));
                    }
                ...
                $emi = array(
                    ...
                    'monthly_emi'             => round($final_emi),
                    'interest_paid'           => round($final_intrst_amt),
