...
class Member extends CI_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->load->model('super_admin/Member_model', 'member_model');
        ...
        if($this->form_validation->run() ==  TRUE) {
            ...
            if ($mls == FALSE) {
                ...
                $use = array(
                    ...
                    'member_id'          => $member_id,
                    'department_type'    => 4,
                    'name'               => $inp['first_name'].' '.$inp['mid_name'].' '.$inp['last_name'],
                    'mobile'             => $inp['mobile'],
                    'email'              => $inp['email'],
                    'address'            => $inp['village'],
    ...
    public function member_data()
    {
        ...
        foreach ($record as $row) {
            ...
            $view = '<a href="javascript:void(0);" data-bs-toggle="modal" data-bs-target=".view_model" style="margin-left:-5px;" onclick="view_member(' . $row->id . ')" title="Click to View Client Details"><i class="fas fa-eye text-success"></i></a>&emsp;
            ...
            $status = ($row->status == 1) ? '
            <a style="color:green; margin-left:-5px;" href="javascript:void()" onClick="return changeStatus(\'' . $row->id . '\',\'0\',\' member\',\'super_admin/common/chageStatus\')" title="Click to Di-Active Client"><b><i class="fa fa-check"></i> </b></a>&emsp;'
            :
            '<a style="color:red; margin-left:-5px;"  href="javascript:void()"  onClick="return changeStatus(\'' . $row->id . '\',\'1\',\' member\',\'super_admin/common/chageStatus\')" title="Click to Active Client"><b><i class="fa fa-times"></i> </b></a>';
    ...
    public function view_member_data() {
        ...
        if($this->input->is_ajax_request()) { 
            ...
            $this->load->view('super_admin/member/view_member', $data);
    ...
    public function update_members() {
        ...
        if($this->input->is_ajax_request()) {
            ...
            $this->load->view('super_admin/member/update_member', $data);
    ...
    public function add_center() {
        ...
        if($this->input->is_ajax_request()) {
            ...
            $this->load->view('super_admin/member/add_center', $data);
